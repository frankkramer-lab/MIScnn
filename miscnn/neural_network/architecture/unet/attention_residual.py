...
#          python_for_microscopists/blob/master/      #
...
# -----------------------------------------------------------------------#
#         Architecture class: Residual Attention U-Net Standard          #
# -----------------------------------------------------------------------#
""" The U-Net architecture with attention and residual connections.
...
def attention_block2D(x, gating, inter_shape):
    ...
    theta_x = Conv2D(filters=inter_shape, kernel_size=3, strides=2, padding='same')(x)  # 16
...
def contracting_layer_2D(input, neurons, ba_norm, ba_norm_momentum):
    ...
    shortcut = Conv2D(filters=neurons, kernel_size=1, padding='same')(input)
    if ba_norm: shortcut = BatchNormalization(momentum=ba_norm_momentum)(shortcut)  # axis=3
    ...
    pool = MaxPooling2D(pool_size=2)(res_path)
...
def expanding_layer_2D(input, neurons, concatenate_link, ba_norm,
    ...
    att = attention_block2D(concatenate_link, gating, neurons)  # Neurons = Filter?
...
def contracting_layer_3D(input, neurons, ba_norm, ba_norm_momentum):
    ...
    if ba_norm: conv2 = BatchNormalization(momentum=ba_norm_momentum)(conv2)
    shortcut = Conv3D(filters=neurons, kernel_size=1, padding='same')(conv2)
    if ba_norm: shortcut = BatchNormalization(momentum=ba_norm_momentum)(shortcut)  # axis=3
    res_path = add([shortcut, conv2])
    res_path = Activation('relu')(res_path)  # Activation after addition with shortcut (Original residual block)
    pool = MaxPooling3D(pool_size=2)(res_path)
    return pool, res_path
